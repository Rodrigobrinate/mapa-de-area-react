import logo from './logo.svg';
import './App.css';
import { useState, useEffect } from 'react';
import axios from 'axios';
import { Link } from 'react-router-dom';
import Header from './components/Header';


function App() {
  const [city, setCity] = useState([])
   
  useEffect(() => {
    axios.get("http://187.94.218.212:6868/").then((response) =>  setCity(
      


        response.data.map((subArray) => {
          return (
            
           <li className='li1'>{subArray.name}
            <li>{
              subArray.user_in_city.map((subitem, i) => {
                return (
                  <li className='li2'>{subitem.user.name}<div><p>{subitem.type}</p><p>{subitem.periodo}</p></div></li>
                );
              })
        
            }</li>
        </li>
     

      )})))

  }, []);




  function deleteColeborator(id){

    axios.post("http://187.94.218.212:6868/delete ",{
      id: id,
      
      
    } ).then((response) => {console.log(response.data)})
    
  }
  function search(item){
    
    axios.post("http://187.94.218.212:6868/search",{
      date: document.getElementById('date').value,
      
      
    }  ).then((response) => setCity(
      


        response.data.map((subArray) => {
          return (
            
           <li className='li1'>{subArray.name}
            <li>{
              subArray.user_in_city.map((subitem, i) => {
                if (localStorage.getItem('token') !== null) {
                  return (
                    <li className='li2'><p className='name'>{subitem.user.name}</p><div><p>{subitem.type}</p><p>{subitem.periodo}</p>
                    
                    </div> <img width="15px" onClick={()=> deleteColeborator(subitem.id)} height="15px" src="data:image/png;base64,"/></li>
                  );
                }
                else {
                  return (
                    <li className='li2'><p className='name'>{subitem.colaborator.name}</p><div><p>{subitem.type}</p><p>{subitem.periodo}</p>
                    
                    </div> </li>
                  );
                }
                return (
                  <li className='li2'><p className='name'>{subitem.colaborator.name}</p><div><p>{subitem.type}</p><p>{subitem.periodo}</p>
                  
                  </div> <img width="15px" onClick={()=> deleteColeborator(subitem.id)} height="15px" src="data:image/png;base64,"/></li>
                );
              })
        
            }</li>
        </li>
     

      )})))
  }


  return (
    <div className="App">
      <Header />
      <input type="date" id="date" className='date' onChange={search} />

      <ul>
     {city}
     </ul>
    </div>
  );
}

export default App;
